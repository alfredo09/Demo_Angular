...
export function fromDateToString(date: Date) {
  if(date){
    ...
    return momentDate.format(formatDB);
  }
  return '';
}
...
export function fromDateStringToDMYString(date: string) {
  if (date) {
    const parsedDate = moment(date, formatDate, true);
    if (parsedDate.isValid()) {
      return parsedDate.format(formatDB);
    }
  }
  return '';
}
